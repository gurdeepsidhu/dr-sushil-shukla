...
if (riskBtn) {
  riskBtn.addEventListener('click', () => {
    ...
    if (risk < 10) {
      ...
      level = 'Moderate Risk';
      color = '#f59e0b';
      message = 'Your risk is moderate. Consider lifestyle changes and schedule a checkup with Dr Sushil Shukla.';
    } else {
      level = 'High Risk';
      color = '#ef4444';
      message = 'Your risk is high. Please consult Dr Sushil Shukla immediately for a complete cardiac evaluation.';
    }
...
document.getElementById('calcAscvd')?.addEventListener('click', () => {
  ...
  else if (score < 20) { level = 'elevated'; color = '#f97316'; msg = 'Elevated risk — please consult Dr Sushil Shukla.'; }
...
function applyCareer(position) {
  ...
  window.open('https://wa.me/9195601461999?text=' + msg, '_blank');
}
...
function addCareerJob() {
  ...
  let jobs = JSON.parse(localStorage.getItem('shukla_career_jobs') || '[]');
  jobs.push({ title, qual, type, loc, badge, desc, date: new Date().toISOString() });
  localStorage.setItem('shukla_career_jobs', JSON.stringify(jobs));
